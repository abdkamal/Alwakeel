...
const phoneShell = s => {
  ...
  if (s.nav === 'بلا قائمة') return `قالب شاشة الهاتف TM0 (412×915، Material 3): انسخه بـ Copy('z3AH5', document, {name, x, y}) ثم احذف من النسخة M.TopBar وM.BottomNav وM.FAB (أبقِ M.StatusBar) وابنِ محتوى مركزيًا داخل الإطار Content (شعار أو رسم توضيحي، عنوان ونص شرح، أزرار M.Button.Filled/Text بعرض كامل في الأسفل).` + common
  if (s.nav === '—') return `قالب شاشة الهاتف TM0 (412×915، Material 3): انسخه بـ Copy('z3AH5', document, {name, x, y}) ثم احذف من النسخة M.BottomNav وM.FAB (شاشة تفاصيل أو نموذج مدفوعة فوق التنقل)، واضبط M.TopBar عبر descendants (عنوان الشاشة، وزر الرجوع بأيقونة arrow-right يمينًا بدل menu)، واملأ الإطار Content بحيث يمتد حتى أسفل الشاشة؛ في النماذج ضع زر الحفظ الرئيسي (M.Button.Filled بعرض كامل) في أسفل المحتوى.` + (s.org ? ' أدرج M.OrgHeader (ref) بين M.StatusBar وM.TopBar.' : '') + common
  return `قالب شاشة الهاتف TM0 (412×915، Material 3): انسخه بـ Copy('z3AH5', document, {name, x, y}). شاشة تبويب رئيسية: اضبط في M.BottomNav عبر descendants العنصر «${s.nav}» وحده نشطًا (كبسولة $primary-soft خلف الأيقونة ونص $primary 600) مع تفعيل الشارات المطلوبة فقط (Badge<N> بالعدد)، واضبط عنوان M.TopBar على اسم الشاشة، وأبقِ M.FAB عند (16,815).` + (s.org ? ' أدرج M.OrgHeader (ref) بين M.StatusBar وM.TopBar (اسم المؤسسة والمكتب).' : '') + common
}
