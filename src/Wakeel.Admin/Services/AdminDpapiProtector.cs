...
/// <summary>
/// The Windows half of <see cref="IPlatformProtector"/> for the administration tool: a wrap bound to
/// this computer by the operating system's own data protection with the local machine scope, plus
/// the fixed application entropy the caller supplies.
/// </summary>
/// <remarks>
/// <para>
/// admin-1 does not itself write a machine wrap — the administration database opens with the
/// password or the printed organisation sheet and nothing else, deliberately, because a machine
/// wrap would mean whoever carries the computer away opens the organisation. It is registered
/// because the setup-file and maintenance work of admin-2 and admin-3 needs a protector to hand to
/// Wakeel.Crypto, and because the tool must never fall back to the do-nothing one on a real machine.
/// </para>
/// <para>
/// It calls <c>crypt32.dll</c> directly rather than through a package, because the shipped package
/// list is fixed (ARCHITECTURE.md §1) and these are two entry points.
/// </para>
/// </remarks>
public sealed class AdminDpapiProtector : IPlatformProtector
{
    ...
    private static byte[] Transform(ReadOnlySpan<byte> input, ReadOnlySpan<byte> entropy, bool protect)
    {
        ...
        try
        {
            ...
            {
                // Every caller treats this one code as "this computer cannot open it" and falls back
                // to the password. The operating system's own numeric reason belongs in neither the
                // message nor the screen (AGREEMENT item 15).
