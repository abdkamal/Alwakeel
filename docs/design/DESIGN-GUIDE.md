...
اعتمد المالك بتاريخ 2026-09-14 المقترح 1 «الختم الرسمي» (البند 54). لوحة العلامة `BRAND — الهوية البصرية` (العقدة s7Urbn أعلى الملف) تحوي الشعار المعتمد ومشتقاته: `AppIcon1` (256 مع معاينات 48/32/16)، `AdaptiveIcon1` (108 مع منطقة الأمان 66)، `Splash1` (480×300)، `PhoneSplash1` (206×457 = نصف 412×915)، `InstallerHeader1` (720×72). تُستخدم هذه المشتقات حرفيًا في البناء (أيقونة exe، أيقونة APK التكيفية، شاشات البداية، ترويسة المثبّت) ولا تُرسم بدائل.

## النص المختلط عربي/إنجليزي (Bidi)

طلب المالك (2026-09-14، البند 55) أن يكون دعم RTL كاملًا حتى عند اختلاط العربية بالإنجليزية. القواعد في ملف Pen:
- الاتجاه الأساسي لكل نص عربي هو RTL مهما كان أول حرف. أي نص يبدأ برمز لاتيني أو رقم ويحوي عربية يبدأ بعلامة `U+200F` (RLM) أو يُلفّ كاملًا بـ`U+2067 … U+2069` (RLI … PDI).
- كل رمز لاتيني أو رقمي داخل جملة عربية (رقم رسمي، PLN-PC-01، اسم ملف وامتداده، v0.21، USB/QR/OCR/WebView2، مبالغ وتواريخ) يُعزل بـ`U+2066 … U+2069` (LRI … PDI) حتى تبقى الأقواس وعلامات الترقيم في مواضعها الصحيحة.
- الأفضل عند الإمكان: وضع الرمز اللاتيني في عقدة نص مستقلة داخل صف أفقي (الترتيب يحدده التخطيط لا خوارزمية bidi)، وهو المعتمد للقيم في الجداول وصفوف مفتاح/قيمة.
- التحقق بلقطة شاشة إلزامي بعد أي تعديل على نص مختلط.
في البناء: `dir="rtl"` على الجذر، `<bdi>` أو `unicode-bidi: isolate` لكل رمز لاتيني وكل قيمة يُدخلها المستخدم، `unicode-bidi: plaintext` للنص الحر، خصائص CSS منطقية فقط (`margin-inline-start` …)، أيقونات الاتجاه معكوسة؛ أندرويد: `LocalLayoutDirection` وBidiFormatter.unicodeWrap؛ Word/PDF: فقرات RTL مع تشغيلات bidi. تُضاف سلاسل مختلطة إلى الاختبارات الآلية للواجهة.

## صفحة اختبار النص المختلط

صفحة `docs/design/bidi-test.html` (ونسخة منشورة على claude.ai) تعرض السطور المختلطة الواردة في الشاشات بمحرّك متصفح حقيقي بلا معالجة ومع العزل، مع قواعد البناء. تُضاف حالاتها إلى اختبارات الواجهة الآلية.
