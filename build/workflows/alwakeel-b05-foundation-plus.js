...
const PACKAGES = [
  {
    ...
    extraCheck: 'the moved static web assets resolve at runtime (staticwebassets.runtime.json of Wakeel.Desktop lists _content/Wakeel.Design/...), scoped CSS bundle still includes every component style, RTL/bidi rules untouched, and Wakeel.Admin.UI can reference the components without pulling Wakeel.Sync/Ocr/Search/Reports',
...
2. Arabic strings: ALL of them live in Wakeel.Design as the single partial static class Wakeel.Design.Text.Ar split over partial files by area (Text/Ar.Common.cs for shared groups such as Fields/Buttons/Dialogs/States/Menu/Pager/Breadcrumb/Nav/Toast, Text/Ar.App.cs for the desktop-app groups such as AttentionCenter/Gallery/Sample/sidebar menu items). Future packages add their own partial file (e.g. Ar.Admin.cs, Ar.FirstRun.cs). Wakeel.UI must contain no Arabic string class of its own after the move; every reference compiles as Ar.X.Y with one using.
3. Update namespaces/usings (_Imports.razor in Wakeel.UI and Wakeel.Admin.UI import Wakeel.Design, Wakeel.Design.Components, Wakeel.Design.Text, Wakeel.Design.Bidi), the ProjectReferences (Wakeel.UI -> Wakeel.Design; Wakeel.Admin.UI -> Wakeel.Design in addition to Core+Crypto), Wakeel.Desktop/wwwroot/index.html (_content/Wakeel.Design/... for tokens.css, app.css, app.js, fonts), and DI registration helpers (a Wakeel.Design AddWakeelDesign() extension for the UI services, called by the desktop host).
4. tests/Wakeel.UI.Tests: fix usings/namespaces so every existing test still passes (133+); the WakeelTestContext helper registers the Design services.
5. Result: 'dotnet build Wakeel.slnx' green with 0 warnings; src/Wakeel.Desktop/bin/Debug/net10.0-windows/Wakeel.Desktop.staticwebassets.runtime.json lists the _content/Wakeel.Design assets; Wakeel.Desktop.styles.css still contains the component styles. Report the final folder layout in notes.
Rules: pure move/rename, no behaviour changes, no new components; keep file history readable (same file names).`,
  },
  {
...
4. RecoveryCode reuse: nothing new; the 16-char package password generator (PackagePassword.New(): 16 chars from an unambiguous alphabet, cryptographically random) lives here too.
5. Tests: round trip with all optional entries, round trip without them, wrong password, tampered payload, tampered manifest, other device, older exportSeq, future date, pinned-key mismatch, revocation list carried through, canonical bytes stable across two writes with the same content and clock (except keySalt/nonces). Keep every existing test green; do not touch docs.`,
  },
  {
    ...
    spec: `Prototype the end-to-end harness that later packages (B2–B8) will use for acceptance screenshots and walkthroughs. ARCHITECTURE.md §11 describes it.
...
log('B0.5: design split and setup format in parallel; E2E harness after the split')
